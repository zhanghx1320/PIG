...
def get_timestep_embedding(timesteps, embedding_dim):
    ...
    """
    assert len(timesteps.shape) == 1, len(timesteps.shape)
...
class MultiDiffusionModel(nn.Module):
    def __init__(self, model_config):
        super().__init__()
        self.config = model_config
        ch, out_channels, ch_mult = model_config.ch, model_config.out_channels, tuple(model_config.ch_mult)
        num_res_blocks = model_config.num_res_blocks
        attn_resolutions = model_config.attn_resolutions
        dropout = model_config.dropout
        in_channels = model_config.in_channels
        resolution = model_config.initial_resolution
        resamp_with_conv = model_config.resamp_with_conv

        self.ch = ch
        self.temb_ch = model_config.temb_ch
        self.num_resolutions = len(ch_mult)
        self.num_res_blocks = num_res_blocks
        self.resolution = resolution
        self.in_channels = 2 * in_channels if model_config.xy_input_type == 'concat' else in_channels
        ...
        # downsampling
        # （提取特征（ResnetBlock*num_res_blocks,增加通道）+注意力+下采样）*num_resolutions
        ...
        for i_level in range(self.num_resolutions):
            ...
            if i_level != self.num_resolutions - 1:
                ...
                curr_res = tuple(np.array(curr_res) // 2)
        ...
        # upsampling
        # （提取特征（ResnetBlock*num_res_blocks,减少通道）+注意力+上采样）*num_resolutions
    ...
    def forward(self, x, cond, t):
        ...
        # downsampling
        x = x + cond if self.config.xy_input_type == 'add' else torch.cat([x, cond], dim=1)
